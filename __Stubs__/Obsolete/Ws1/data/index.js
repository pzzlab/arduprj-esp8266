const NZONE = 6, NPRG = 8, NPWR = 6, NINOUT = 3;
...
function DecodeSet(http)
{
	...
	TAdj = a.slice(ofs,ofs + NZONE); ofs += NZONE;
	// the Power names
...
function ShowHeat(num,zone)
{
	...
	ta = TAct[zone]; ts = Pgms[num][PIdx]; if (TMan[zone*2]) ts = TMan[zone*2+1]; 
  e = document.getElementById('Temp' + Number(zone));	
	...
	if (TMan[zone*2]) 
	 ...
	 else
		{
			...
			for (n = PIdx; n < 48; n++) if (Pgms[num][n] != ts) break;		// n contain the first changed value
...
function ShowPwr(num)
{
	...
	sc = h / Wmax[num];					c.clearRect(0,0,wh,h);	c.font = "bold 20px courier new";
	w = 20;		wa = W[num] / 10; wp = Wpk[num] / 10;			wm = Wmax[num];
	...
	c.fillStyle = 'orange'; c.fillRect(0, h - wa * sc, w , -(wp - wa) * sc);	c.fillText(wp.toFixed(1) + ' W', wh, h - 82);
	c.fillStyle = 'black'; 
